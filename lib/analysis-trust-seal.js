...
/**
 * Cœur scellé : les objets qui portent les signaux de confiance.
 *  - financial_data : revenu (candidat) ;
 *  - trust_and_security : sceau Visale / 2D-Doc, score de fraude ;
 *  - document_metadata : type + owner_name (utilisé pour la concordance d'identité du GARANT).
 * v2 (audit garant) : document_metadata ajouté au cœur — un client ne peut plus forger owner_name
 * (concordance de nom) ni digital_seal pour certifier un garant à partir de signaux fabriqués.
 */
function canonicalAnalysisCore(src) {
  const a = src || {};
  // Normalisation par aller-retour JSON : SIGN (serveur, objet en mémoire) et VERIFY (client,
  // APRÈS JSON.parse du transport) doivent voir la MÊME représentation. JSON.stringify supprime
  // les `undefined` — sans cette normalisation, un document LÉGITIME comportant un champ optionnel
  // undefined verrait son sceau échouer à la sauvegarde et serait neutralisé à tort (bug détecté
  // par le test d'intégration C1 : aller-retour JSON).
  let norm;
  try {
    norm = JSON.parse(
      JSON.stringify({
        ...
        document_metadata: a.document_metadata || {},
      })
    );
  } catch (_e) {
    norm = { v: 2, financial_data: {}, trust_and_security: {}, document_metadata: {} };
  }
