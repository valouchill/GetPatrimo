      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-600 transition-colors hover:bg-amber-100"
          >
        ...
        {room.equipment.length === 0 && (
          <button
            type="button"
            onClick={() => setShowPicker(true)}
            className="w-full rounded-xl border-2 border-dashed border-slate-200 py-6 text-center text-sm text-slate-400 transition-colors hover:border-amber-300 hover:text-amber-500"
          >
        ...
        {room.equipment.map((eq, i) => (
          <div key={eq.name + i} className="rounded-xl border border-slate-200 bg-white p-3 space-y-2">
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-200 px-3 py-2 text-xs outline-none focus:border-amber-300 focus:ring-2 focus:ring-amber-100"
            />
          </div>
        ))}
      </div>
